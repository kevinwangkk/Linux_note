...
函数功能：
	/*复制*/当前正在调用进程的方式来启动一个新进程，而其中新启动的进程叫做子进程，原来的进程叫做父进程，函数调用成功时/*父进程返回子进程的进程号，子进程返回0*/，函数调用失败时直接返回-1，没有子进程被创建；
...
（2）父子进程之间的关系
	...
	2. 如果父进程先于子进程结束，则子进程变成孤儿进程，子进程会变更父进程（一般重新设定init(1)为新的父进程），init进程收养了孤儿进程，所以init叫做孤儿院；
	3. 如果子进程先于父进程结束，但是父进程由于各种原因没有接收到子进程发来的信号，也就是没有回收子进程的资源，但是子进程已经结束了，因此子进程变成了僵尸进程；
...
（3）父子进程的复制(内存)关系
	...
	使用fork函数创建子进程后，子进程会复制父进程中的文件描述符总表，但是不会复制文件表结构，使得/*父子进程中的文件描述符对应同一个文件表结构*/；
...
函数功能：
	用于立即终止当前正在调用的进程，关闭所有属于当前进程的打开的文件描述符；让该进程的所有子进程变更父进程为init进程；给父进程发送SIGCHLD信号让父进程帮其进行善后工作
	参数status的数值被返回给当前进程的父进程作为当前进程的退出状态信息，如果父进程收集该状态信息，则需要调用wait系列函数进行收集；
...
函数功能：
	...
	该函数创建子进程时不会复制父进程中的内存空间，而是/*直接占用*/导致父进程被挂起，直到子进程终止或者调用exec系列函数为止，而子进程终止的方式不可以是：从当前函数返回 以及 调用exit()函数，而/*应该调用_exit()函数来终止*/；
...
作业：
	使用fork函数创建子进程，在子进程中申请一个int类型大小的动态内存，提示用户输入半径，根据用户的输入计算周长并打印，要求当子进程终止时自动释放动态内存，父进程等待子进程结束，并获取退出状态信息，打印出来；
