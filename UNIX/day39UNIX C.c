...
基本概念
	...
	进程是重量级的，每个进程都需要独立的内存空间，因此新建进程对于资源的消耗比较大；而线程是轻量级的，新建线程会共享所在进程的内存资源，但是每个线程都拥有一块独立的栈区；
...
/*2.线程之间的关系*/
	...
	当子线程创建成功后，两个线程各自独立运行，子线程执行对应的线程处理函数，主线程继续向下执行，两个线程之间的执行先后次序由操作系统调用算法决定；
...
练习：
	使用pthread_create函数创建一个子线程，在子线程的线程处理函数中根据参数传入的半径计算周长和面积并打印出来，而其中存放半径的内存空间由main函数中调用sbrk函数申请，并由用户手动输入；
	vi 02circle.c 
...
函数功能：
	主要用于/*等待参数thread指定的线程终止*/，如果参数thread指向的线程终止了，则该函数立即返回，当然要求该线程必须是可汇合的/可等待的；
...
练习：
	使用pthread_create函数创建子线程，在线程处理函数中计算1～100之间的和并放在变量sum中，返回sum变量的地址，在主线程中调用pthread_join函数等待子线程结束，并获取退出状态信息，打印出来；
	vi 04join.c
...
函数功能：
	主要用于将参数指定的线程设置为分离状态的线程，当一个分离状态的线程终止时，它的资源会被自动释放给系统，不需要其他线程来汇合/帮助，也就是分离状态的线程无法被pthread_join函数所等待/汇合；
...
函数功能：
	主要用于终止当前正在调用的线程，并通过参数返回当前线程的退出状态信息，可以使用同一个进程中的其他线程调用pthread_join函数来获取该退出状态信息；

练习：vi 05exit.c文件，启动子线程，子线程的线程处理函数中负责打印1～20之间的整数，每隔一秒打印一次，当i的值为10时，终止当前线程，并返回i的数值，主线程负责等待并获取退出状态信息，最后打印出来
...
函数功能：
	主要用于给参数指定的线程发送取消的请求，默认情况下是可以被取消的，是否被取消以及何时被取消可以通过以下两个函数进行设置：
