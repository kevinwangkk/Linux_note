...
/*标C和UC文件操作函数的比较*/
	标C的文件操作函数执行效率高于UC的文件操作函数，因为标C的文件操作函数内部提供了输入输出缓冲区，当数据积累到一定数量之后才去访问内核，才会将数据写入到文件中；
...
/*文件描述符的工作原理*/
	...
	也就是从0开始，其中 0 1 2 已经被系统占用，分别代表标准输入，标准输出以及标准错误，
	因此一般从 3 开始使用，文件描述符的最大值可以到OPEN_MAX(当前教学环境是1024)；
...
注意：
	复制文件描述符的本质就是复制文件描述所对应的文件表地址信息，使得多个文件描述符可以对应同一个文件，因此无论使用哪个文件描述符都可以访问文件；
