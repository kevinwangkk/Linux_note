...
结论：
	操作系统中提供的信号集类型是：/*sigset_t*/ 类型，底层是一个超级大的整数，采用每一个二进制位来代表该信号是否存在，其中0表示该信号不存在，1表示信号存在；
...
注意：
	...
	而对于不可靠信号来说（1-31），无论发送多少次，排队的信号只有1个当信号屏蔽被解除，相当于将阻挡的隔板移开，因此所有阻挡的信号都会被依次处理；
...
捕获到了信号50
捕获到了信号3
捕获到了信号2 	//会按到从大到小的顺序(数字大小)  即使先发信号2 也是先捕获到信号3
