...
/*使用fcntl函数实现文件锁的功能*/
1. 文件锁的由来
	当多个进程在同一个时刻向同一个文件中的同一块区域写入不同数据时，可能会造成文件数据的交错和混乱文件，理论上多个进程读文件可以同时进行，但是只要有一个进程执行写操作，那么多个进程就不同时进行，为了实现该效果，需要借助文件锁机制；

	文件锁本质就是读写锁，一把读锁和一把写锁，其中读锁是共享锁，允许其他进程加读锁但不允许加写锁；而写锁是互斥锁，不允许其他进程加读锁和写锁；
...
2. 使用F_SETLK作为函数第二个实参的用法
	...
	当文件被加写锁时，依然是可以写入数据到文件中，由此可见，文件锁并不能真正锁定对文件的读写操作，只能锁定其他的锁，也就是导致第二次加锁失败（两个读锁除外） => 君子协定
